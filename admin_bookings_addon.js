...
function toTimestampMs(value) {
  try {
    ...
    if (value instanceof Date) {
      return !Number.isNaN(value.getTime()) ? value.getTime() : 0;
    }
...
function formatDateTimeField(dateField) {
  try {
    ...
    if (typeof dateField.toDate === 'function') {
      ...
      d = dateField;
    } else if (typeof dateField === 'number' || typeof dateField === 'string') {
  ...
  if (rooms > 0) {
    return `
      <div style="display:flex; flex-direction:column; line-height:1.4;">
        ...
        <span style="font-size:0.75rem; color:var(--text-muted);">${adults} بالغين${children > 0 ? ` + ${children} أطفال` : ''}</span>
      </div>
    `;
  }
...
function buildPaymentHtml(booking) {
  ...
  if (paymentMethod === 'transfer') {
    return {
      badge: `
        <span style="font-size:0.7rem; background:#fef3c7; color:#d97706; padding:2px 6px; border-radius:4px; font-weight:700; margin-inline-start:6px;">
      ...
      receipt: receiptUrl
        ? `
          <a href="${escapeHtml(receiptUrl)}" target="_blank" rel="noopener noreferrer"
             style="background:#fef3c7; color:#d97706; padding:6px 12px; border-radius:8px; font-size:0.8rem; font-weight:700; text-decoration:none; display:inline-flex; align-items:center; gap:6px; border:1px solid #fde68a;">
  ...
  return {
    badge: `
      <span style="font-size:0.7rem; background:#ecfdf5; color:#059669; padding:2px 6px; border-radius:4px; font-weight:700; margin-inline-start:6px;">
...
function buildNotesHtml(booking) {
  ...
  return `
    <div style="margin-top:10px; font-size:0.8rem; color:var(--text-muted); background:#fffbeb; padding:8px 10px; border-inline-start:3px solid #f59e0b; border-radius:0 6px 6px 0;">
...
function buildArrivalHtml(booking) {
  ...
  return `
    <div style="display:flex; align-items:center; gap:10px; font-size:0.9rem; color:var(--text-main);">
      ...
      <span>وقت الوصول المتوقع: <strong style="color:var(--primary);">${escapeHtml(arrivalTime)}</strong></span>
    </div>
  `;
}
...
function renderBookingCard(doc) {
  ...
  const occupancyHtml = buildOccupancyHtml(booking);
  const addonsHtml = buildAddonsHtml(booking);
  const notesHtml = buildNotesHtml(booking);
  const arrivalHtml = buildArrivalHtml(booking);
  ...
  return `
    <div class="booking-card"
         ...
         style="background:var(--surface-color); border:1px solid var(--border-color); border-radius:16px; padding:20px; display:flex; flex-direction:column; gap:12px; box-shadow:var(--shadow-soft);">

      <div style="display:flex; justify-content:space-between; align-items:flex-start; border-bottom:1px solid var(--border-color); padding-bottom:14px; gap:12px;">
        <div style="min-width:0;">
          <div style="font-size:1.15rem; font-weight:700; color:var(--text-main); margin-bottom:4px; word-break:break-word;">
            ${guestName}
          </div>
          <div style="margin-bottom:6px;">${payment.badge}</div>
          <div style="font-size:0.75rem; color:var(--text-muted); font-family:monospace; background:var(--bg-color); padding:3px 8px; border-radius:6px; display:inline-block;">
            رقم: #${docIdShort}
          </div>
        </div>
        ...
        <div style="display:flex; flex-direction:column; align-items:flex-end; gap:6px; flex-shrink:0;">
          <span class="status-badge ${status.className}"
                style="font-size:0.75rem; font-weight:700; padding:6px 12px; border-radius:8px; background:${status.bg}; color:${status.color}; border:1px solid ${status.color}40;">
      ...
      <div style="display:flex; flex-direction:column; gap:12px;">
        ...
        <div style="display:flex; align-items:flex-start; gap:10px; font-size:0.9rem; color:var(--text-main);">
          ...
          <div style="display:flex; flex-direction:column; gap:3px;">
            <span style="font-weight:600;">${checkIn} <i class="ph ph-arrow-left" style="font-size:0.7rem; color:var(--text-muted); margin:0 4px;"></i> ${checkOut}</span>
            <span style="font-size:0.75rem; color:var(--text-muted); font-weight:600; background:#f1f5f9; padding:2px 8px; border-radius:4px; display:inline-block; width:fit-content;">
              ${nights} ليالٍ
            </span>
          </div>
        </div>
        ...
        <div style="display:flex; align-items:center; gap:10px; font-size:0.9rem; color:var(--text-main); min-width:0;">
          <i class="ph-fill ph-phone" style="color:var(--text-muted); font-size:1.2rem;"></i>
          <span dir="ltr" style="font-family:monospace; font-weight:600; overflow-wrap:anywhere;">${guestPhone}</span>
        </div>
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; gap:12px; margin-top:8px; padding-top:14px; border-top:1px dashed var(--border-color); flex-wrap:wrap;">
      ...
      ${statusKey === 'pending' ? `
        <div class="booking-actions-row" style="display:flex; gap:10px; margin-top:16px; padding-top:16px; border-top:1px solid var(--border-color);">
          <button type="button"
                  ...
                  style="flex:1; display:flex; align-items:center; justify-content:center; gap:8px; padding:10px; border-radius:10px; font-weight:700; font-size:0.9rem; cursor:pointer; background:#ecfdf5; color:#059669; border:1px solid #10b981; font-family:inherit;">
          ...
          <button type="button"
                  ...
                  data-doc-id="${escapeHtml(doc.id)}"
                  style="flex:1; display:flex; align-items:center; justify-content:center; gap:8px; padding:10px; border-radius:10px; font-weight:700; font-size:0.9rem; cursor:pointer; background:#fef2f2; color:#e11d48; border:1px solid #ef4444; font-family:inherit;">
...
async function loadBookings() {
  ...
  try {
    ...
    try {
      ...
      console.warn('[loadBookings] Fallback to client-side sorting:', orderErr);
      const fallbackSnap = await db.collection('bookings').get();
      docs = fallbackSnap.docs.sort((a, b) => {
        const aMs = toTimestampMs((a.data() || {}).createdAt);
        const bMs = toTimestampMs((b.data() || {}).createdAt);
        return bMs - aMs;
      });
    }
    ...
    let html = `
      <div class="bookings-grid"
           style="display:grid; grid-template-columns:repeat(auto-fill, minmax(320px, 1fr)); gap:20px; width:100%;">
    `;

    docs.forEach(doc => {
      html += renderBookingCard(doc);
    });
    ...
    container.onclick = event => {
      ...
      const newStatus = button.dataset.bookingAction;
      window.updateBookingStatus(docId, newStatus);
    };
  } catch (err) {
    console.error(err);
    container.innerHTML = `
      <div style="text-align:center; padding:40px; color:#e11d48; background:#ffe4e6; border-radius:12px; font-weight:bold;">
